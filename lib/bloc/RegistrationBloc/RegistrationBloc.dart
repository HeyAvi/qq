...
class RegistrationBloc extends Bloc<RegistrationEvent, RegistrationState> {
  ...
  RegistrationBloc(this.repository)
      : super(const RegistrationInitialState(version: 0)) {
  ...
  void _handleRegisterEvent(
    ...
    if (state.status!.isValidated) {
      ...
      Response? serverAPIResponseDto = await repository.register(
          event.context, event.name, event.email, event.phone);
      ...
      if (serverAPIResponseDto != null) {
        if (serverAPIResponseDto.data["status"].toString() == "200" ||
          ...
          prefs.setString(
              "userDataKey", serverAPIResponseDto.data["data"].toString());
          prefs.setString("userId", serverAPIResponseDto.data["id"].toString());
          RegistrationCompleteState completeState = RegistrationCompleteState(
  ...
  void _handleEmailChanged(
      EmailChanged event, Emitter<RegistrationState> emit) {
  ...
  void _handleMobileNumberChanged(
      MobileNumberChanged event, Emitter<RegistrationState> emit) {
