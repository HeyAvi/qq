...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  ProfileBloc(this.repository) : super(const ProfileInitialState(version: 0)) {
    on<GetProfileDataEvent>(_handleGetProfileDataEvent,
        transformer: sequential());
    on<UpdateProfileDataEvent>(_handleUpdateProfileDataEvent,
        transformer: sequential());
  }

  void _handleGetProfileDataEvent(
      GetProfileDataEvent event, Emitter<ProfileState> emit) async {
    Response? serverAPIResponseDto =
        await repository.getProfileData(event.context);
    if (serverAPIResponseDto != null &&
      ...
      userDataService.setUserdata(
          newData,
          dataDto["wallet"]["amount"].toString(),
          dataDto["tickets"]["ticket_id"].toString(),
          dataDto["total_contest"]["contest_x_user_id"].toString());
      print(userDataService.userData.toString());
      ProfileCompleteState completeState = ProfileCompleteState(
          context: event.context, version: state.version + 1);
      emit(completeState);
    }
  }

  void _handleUpdateProfileDataEvent(
      UpdateProfileDataEvent event, Emitter<ProfileState> emit) async {
    DialogUtil.showProgressDialog("", event.context);
    Response? serverAPIResponseDto = await repository.updateProfileData(
        event.context,
        event.name,
        event.gender,
        event.dob,
    ...
    if (serverAPIResponseDto != null) {
      ...
      Userdata newData = Userdata.fromJson(dataDto["data"]);
      UserDataService userDataService = getIt<UserDataService>();
      userDataService.setUserdata(newData, userDataService.walletAmount,
          userDataService.totalTickets, userDataService.totalParticipation);
      print(userDataService.userData.toString());
