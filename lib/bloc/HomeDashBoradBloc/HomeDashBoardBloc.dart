...
class HomeDashBoardBloc extends Bloc<HomeDashBoardEvent, HomeDashBoardState> {
  ...
  void _handleFetchContestEvent(
      FetchContestEvent event, Emitter<HomeDashBoardState> emit) async {
    Response? serverAPIResponseDto =
        await repository.getContestData(event.context, event.userId);
  ...
  void _handleGetLastContestDataEvent(
      GetLastContestDataEvent event, Emitter<HomeDashBoardState> emit) async {
    Response? serverAPIResponseDto =
        await repository.getLastContestData(event.context, event.date);
    if (serverAPIResponseDto != null) {
      if (serverAPIResponseDto.data["status"].toString() == "200" ||
        ...
        List<ContestUserData>? contestUserDataList =
            (serverAPIResponseDto.data!["data"] as List)
                .map((itemWord) => ContestUserData.fromJson(itemWord))
                .toList();
        HomeDashBoardCompleteState completeState = HomeDashBoardCompleteState(
            context: event.context,
            version: state.version + 1,
            contestdata: event.contestdata,
            contestUserDataList: contestUserDataList);
        emit(completeState);
  ...
  void _handleBottomIndexChange(
      BottomIndexChange event, Emitter<HomeDashBoardState> emit) async {
    BottomIndexChangedState completeState = BottomIndexChangedState(
        context: event.context,
        version: state.version + 1,
        bottomIndex: event.currentIndex);
    emit(completeState);
  }
}
